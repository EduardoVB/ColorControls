...
Public Function FileExistsK(ByVal PathName As String) As Boolean
On Error Resume Next
Dim Attributes As VbFileAttribute, ErrVal As Long
Attributes = GetAttr(PathName)
ErrVal = Err.Number
On Error GoTo 0
If (Attributes And (vbDirectory Or vbVolume)) = 0 And ErrVal = 0 Then FileExistsK = True
End Function
...
Public Function IsFormLoadedK(ByVal FormName As String) As Boolean
Dim i As Integer
...
Public Function GetWindowClassNameK(ByVal hWnd As Long) As String
Dim Buffer As String, RetVal As Long
Buffer = String(256, vbNullChar)
RetVal = GetClassName(hWnd, StrPtr(Buffer), Len(Buffer))
If RetVal <> 0 Then GetWindowClassNameK = Left$(Buffer, RetVal)
End Function
...
Public Sub SetWindowRedrawK(ByVal hWnd As Long, ByVal Enabled As Boolean)
...
Public Function InIDEK(Optional ByRef B As Boolean = True) As Boolean
If B = True Then Debug.Assert Not InIDEK(InIDEK) Else B = True
End Function
